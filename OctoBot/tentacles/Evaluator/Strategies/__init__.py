...
if check_tentacle_version('1.2.0', 'level_strategy_evaluator', 'OctoBot-Tentacles'):
    try:
        from .level_strategy_evaluator import *
    except Exception as e:
        get_logger('TentacleLoader').error(f'Error when loading level_strategy_evaluator: '
                                           f'{e.__class__.__name__}{f" ({e})" if f"{e}" else ""}. If this '
                                           f'error persists, try reinstalling your tentacles via '
                                           f'"python start.py tentacles --install --all".')
