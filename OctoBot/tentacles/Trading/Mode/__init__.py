...
if check_tentacle_version('1.2.0', 'larre_trading_mode', 'OctoBot-Tentacles'):
    try:
        from .larre_trading_mode import *
    except Exception as e:
        get_logger('TentacleLoader').error(f'Error when loading larre_trading_mode: '
                                           f'{e.__class__.__name__}{f" ({e})" if f"{e}" else ""}. If this '
                                           f'error persists, try reinstalling your tentacles via '
                                           f'"python start.py tentacles --install --all".')
